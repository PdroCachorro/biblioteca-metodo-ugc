...
CARDS = {
    ...
    "ugc_celular": {"tag": "GANCHO", "title": "Print no celular", "tool": "Kling 3 / Veo 3", "reqs": ["model", "screenshot", "scene"],
        ...
        "motion_prompt": "Animate this exact photo. The hand holding the phone stays fixed in place the entire time, never letting go, never lowering it, holding it steady in exactly the same position from start to finish. With her other hand — the one not holding the phone — she points at the screen to highlight the product, glances back at the camera with a playful head tilt and briefly touches her collarbone, then brings that same other hand up covering the lens like a wipe transition; the phone-holding hand never moves and the phone never floats or changes position on its own. Keep her facial expression subtle and consistent with the original photo throughout the whole clip — no exaggerated expression changes that could make her look different from the reference. Static locked-off camera, identical framing and distance from the first frame to the last — absolutely no dolly-in, no push-in, no zoom of any kind — only natural handheld phone-camera wobble in place. ~5-6 seconds. No on-screen text, no captions, no subtitles, no watermarks."},
